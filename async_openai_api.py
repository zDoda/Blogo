...
async def create_image(prompt, image_size="1792x1024"):
    """
...
async def chat_completion(prompt: str, config) -> str:
...
async def image_gen(blog_post_idea, config):
    image_prompt_prompt = f'''
    Write a image prompt that relates with {blog_post_idea}, has
    a {random.choice(art_styles)}
    '''
    image_prompt = await chat_completion(image_prompt_prompt, config)
    image_src = await create_image(image_prompt)
    return image_src
...
async def wait_for_run_completion(thread_id, run_id, timeout=300):
...
async def process_blog_para(thread_id,
                            blog_post_idea,
                            writer_id,
                            config,
                            header,
    ...
    await wait_for_run_completion(thread_id, article_run.id)
...
async def gen_all_images(blog_post_idea, config, headers):
    image_tasks = [image_gen(blog_post_idea,
                             config,
                             i) for i in range(0, len(headers))]
...
# Blog Post Writer
async def process_blog_post(thread_id, blog_post_idea, outline_id, writer_id, slug, config):
    # Generate outline
    outline_request = f'''
    ...
    if outline:
        ...
        post_tasks = [process_blog_para(thread_id,
                                        blog_post_idea,
                                        writer_id,
                                        config,
                                        header,
                                        outline,
                                        header_idx) for header_idx, header in enumerate(headers)]
        post_responses = await asyncio.gather(*post_tasks)
        post_responses.sort(key=lambda x: x[0])
        for order, para in post_responses:
            article += para
...
def process_content_plan(outline_ass, writer_ass, config):
    ...
    with open(input_file, newline='', encoding='utf-8') as csvfile:
        ...
        for row in tqdm(reader, desc="Processing Blog Posts"):
            ...
            outline, article = asyncio.run(process_blog_post(
                thread_id,
                blog_post_idea,
                outline_ass.id,
                writer_ass.id,
